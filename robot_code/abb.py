...
class Robot:
    ...
    def buffer_add_extax(self, pose):
        '''
        Uses format [[x,y,z],[q1,q2,q3,a4],extax]
        '''
        msg = "30 "
        ...
        msg += format(pose[2],"+08.5f")
        msg += " #"
        self.send(msg)

    def buffer_set(self, pose_list, includesExtAx = False):
        '''
        Adds every pose in pose_list to the remote buffer
        '''
        self.clear_buffer()
        for pose in pose_list: 
            if not includesExtAx:
                self.buffer_add(pose)
            else:
                self.buffer_add_extax(pose)
    ...
    def moveC_wExt(self,pose,extAx):
        msg = "101 "
        for cartesian in pose[0]:
            msg += format(cartesian * self.scale_linear,  "+08.1f") + " " 
        for quaternion in pose[1]:
            msg += format(quaternion, "+08.5f") + " " 
        msg += format(extAx,"+08.5f")
        msg += " #"
        return self.send(msg)
